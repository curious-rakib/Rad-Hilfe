...
const Home = () => {
	const { bikeDetails, dailyCommute, recreationalCommute } = useAppSelector((state) => state.rootSetBikeReducer);
	const bikeInfo = {
		...bikeDetails,
		dailyCommute,
		recreationalCommute,
	...
	useEffect(() => {
		...
		const health = async () => {
			const healthD = await bicycleHealth('64dcc8ecfce64359274ced75');
